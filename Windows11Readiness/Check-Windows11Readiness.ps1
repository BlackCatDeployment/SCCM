<#
    ...
    .OUTPUTS
    Write a registry key "HKLM:\SOFTWARE\BlackCat" value "Win11Ready" with blocable reason or Ready
    .AUTHOR
    Florian VALENTE
    .NOTES
...
If ($BuildNumber -ge 22000) {
    ...
    Set-ItemProperty $RegPath -Name $RegName -Value "Ready" -Force | Out-Null
    exit 0
}
...
if ($Results.psobject.properties.value -contains $false) {
    ...
    Set-ItemProperty $RegPath -Name $RegName -Value (($Results.psobject.Properties | ? {$_.value -eq $false}).Name -join ", ") -Force | Out-Null
}
else {
